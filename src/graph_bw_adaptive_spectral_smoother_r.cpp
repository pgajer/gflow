#include <R.h>                  // For Rprintf, error, etc.
 ...
 *   - bw_mean_abs_errors:    Mean absolute error for each bandwidth
...
SEXP S_graph_bw_adaptive_spectral_smoother(
	...
	auto mat_to_sexp = [&protect_count](const std::vector<std::vector<double>>& M) {
		...
		SEXP x = PROTECT(allocMatrix(REALSXP, nrow, ncol)); protect_count++;
		double* ptr = REAL(x);
		for (size_t j = 0; j < ncol; ++j) {
			if (M[j].size() != nrow)
				error("Inconsistent bw_predictions dimensions");
