...
SEXP S_wmabilo(SEXP s_x,
    ...
    mabilo_t wmabilo_results = wmabilo(x,
                                       y,
                                       y_true,
                                       w,
                                       k_min,
    ...
    SET_VECTOR_ELT(result, 3, PROTECT(convert_vector_double_to_R(wmabilo_results.k_mean_errors))); n_protected++;

    // true errors
    if (y_true_exists) {
        SET_VECTOR_ELT(result, 4, PROTECT(convert_vector_double_to_R(wmabilo_results.k_mean_true_errors))); n_protected++;
    } else {
        SET_VECTOR_ELT(result, 4, R_NilValue);
    }

    SET_VECTOR_ELT(result, 5, PROTECT(convert_vector_double_to_R(wmabilo_results.predictions))); n_protected++;
    SET_VECTOR_ELT(result, 6, PROTECT(convert_vector_vector_double_to_R(wmabilo_results.k_predictions))); n_protected++;
...
SEXP S_mabilo(SEXP s_x,
    ...
    if (n_bb > 0) {
        SET_VECTOR_ELT(result, 7, PROTECT(convert_vector_double_to_R(wmabilo_results.bb_predictions))); n_protected++;
        SET_VECTOR_ELT(result, 8, PROTECT(convert_vector_double_to_R(wmabilo_results.cri_L))); n_protected++;
        SET_VECTOR_ELT(result, 9, PROTECT(convert_vector_double_to_R(wmabilo_results.cri_U))); n_protected++;
    } else {
...
SEXP S_mabilo_with_smoothed_errors(SEXP s_x,
    ...
    mabilo_t mabilo_with_smoothed_errors_results = mabilo_with_smoothed_errors(x,
                                                                               y,
                                                                               y_true,
                                                                               k_min,
                                                                               k_max,
    ...
    SET_VECTOR_ELT(result, 3, PROTECT(convert_vector_double_to_R(mabilo_with_smoothed_errors_results.k_mean_errors))); n_protected++;

    SET_VECTOR_ELT(result, 4, PROTECT(convert_vector_double_to_R(mabilo_with_smoothed_errors_results.smoothed_k_mean_errors))); n_protected++;

    // true errors
    if (y_true_exists) {
        SET_VECTOR_ELT(result, 5, PROTECT(convert_vector_double_to_R(mabilo_with_smoothed_errors_results.k_mean_true_errors))); n_protected++;
    } else {
        SET_VECTOR_ELT(result, 5, R_NilValue);
    }

    SET_VECTOR_ELT(result, 6, PROTECT(convert_vector_double_to_R(mabilo_with_smoothed_errors_results.predictions))); n_protected++;
    SET_VECTOR_ELT(result, 7, PROTECT(convert_vector_vector_double_to_R(mabilo_with_smoothed_errors_results.k_predictions))); n_protected++;
