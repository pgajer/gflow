...
/**
 * @brief Performs k-fold cross-validation for Graph Path Linear Models (PGMALOG)
 *
 * @details This function evaluates PGMALOG prediction performance using k-fold cross-validation.
 * For each iteration, it:
 * 1. Randomly selects test vertices
 * 2. Sets their weights to 0 (effectively excluding them from model fitting)
 * 3. Fits PGMALOG using remaining vertices as training data
 ...
 *
 * @note
 * - Uses pgmalog_with_cv_weights() for model fitting with test set exclusion
 ...
 *
 * auto cv_errors = pgmalog_cv(path_graph, y, n_CVs, n_CV_folds, seed,
 *                         1, 2, 1.01, 1e-8);
 * @endcode
 *
...
std::vector<double> pgmalog_cv(
    ...
    for (int cv = 0; cv < n_CVs; ++cv) {
        ...
        auto res = pgmalog_with_cv_weights(path_graph,
                                        y,
                                        weights,
                                        ikernel,
...
/**
 * @brief Performs parallel cross-validation for Gaussian Process Latent Model (PGMALOG)
 *
 * @details This function implements a parallel cross-validation procedure for PGMALOG
 ...
 * @param ikernel Kernel function identifier for the PGMALOG
 ...
 *
 * auto cv_errors = pgmalog_cv_parallel(graph, observations, n_cvs, n_folds,
 *                                  seed, 1, 2, 1.01, 1e-8);
 * ```
 */
