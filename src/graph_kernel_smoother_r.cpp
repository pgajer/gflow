...
SEXP S_graph_kernel_smoother(
    ...
    SEXP s_verbose)
{
    // ---- Graph inputs (LENGTH-first; no long vectors) ----
    ...
    const int Ny = Rf_length(s_y);
    std::vector<double> y(REAL(s_y), REAL(s_y) + (Ny > 0 ? Ny : 0));

    if ((size_t)Ny != adj_list.size()) {
        Rf_error("length(y) (%d) must equal length(adj_list) (%zu).", Ny, adj_list.size());
    }

    const int N = Ny; // convenience
    ...
    if (!(max_bw_factor > min_bw_factor)) Rf_error("max_bw_factor must be > min_bw_factor.");
    ...
    // ---- Build result (container-first; PROTECT/UNPROTECT pairing) ----
    SEXP r_result = PROTECT(Rf_allocVector(VECSXP, 6));                 // [1]

    // names
    {
        SEXP r_names = PROTECT(Rf_allocVector(STRSXP, 6));              // [2]
    ...
    // 0: predictions
    {
        SEXP s = PROTECT(convert_vector_double_to_R(result.predictions)); // [3]
        if ((int) result.predictions.size() != N) {
    ...
    if (with_bw_predictions && !result.bw_predictions.empty()) {
        const int ncol = (int) result.bw_predictions.size();
        const int nrow = (int) result.bw_predictions[0].size();
        if (nrow != N) {
            UNPROTECT(1); // r_result
            Rf_error("bw_predictions: row count (%d) must equal length(y) (%d).", nrow, N);
        }
        for (int j = 1; j < ncol; ++j) {
            if ((int) result.bw_predictions[(size_t)j].size() != nrow) {
                UNPROTECT(1); // r_result
                Rf_error("bw_predictions: column %d has length %d; expected %d.",
                         j + 1, (int) result.bw_predictions[(size_t)j].size(), nrow);
            }
        }

        SEXP s = PROTECT(Rf_allocMatrix(REALSXP, nrow, ncol));             // [4]
        double* p = REAL(s);
        for (int j = 0; j < ncol; ++j) {
            const auto& col = result.bw_predictions[(size_t)j];
            for (int i = 0; i < nrow; ++i) {
                p[i + j * nrow] = col[(size_t)i];
            }
        }
    ...
    // 2: bw_mean_abs_errors
    {
        if (!result.bw_mean_abs_errors.empty() &&
            (int) result.bw_mean_abs_errors.size() != n_bws) {
            UNPROTECT(1); // r_result
            Rf_error("bw_mean_abs_errors length (%d) must equal n_bws (%d).",
                     (int) result.bw_mean_abs_errors.size(), n_bws);
        }
        SEXP s = PROTECT(convert_vector_double_to_R(result.bw_mean_abs_errors)); // [5]
    ...
    // 3: vertex_min_bws
    {
        if (!result.vertex_min_bws.empty() &&
            (int) result.vertex_min_bws.size() != N) {
            UNPROTECT(1); // r_result
            Rf_error("vertex_min_bws length mismatch.");
        }
        SEXP s = PROTECT(convert_vector_double_to_R(result.vertex_min_bws)); // [6]
        SET_VECTOR_ELT(r_result, 3, s);
        UNPROTECT(1); // s                                                   [-]
    }

    // 4: opt_bw_idx (1-based)
    {
        SEXP s = PROTECT(Rf_ScalarInteger((int) result.opt_bw_idx + 1));    // [7]
    ...
    // 5: buffer_hops_used
    {
        SEXP s = PROTECT(Rf_ScalarInteger((int) result.buffer_hops_used));  // [8]
