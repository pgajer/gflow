...
amagelo_t amagelo(
    ...
    if (result.local_extrema.size() > 2) {
        ...
        // debugging
        // Rprintf("In result.local_extrema.size() > 2 block!!!");
        ...
        // Step 2: Parameters
        const double small_depth_threshold = 0.01;  // you can tune this threshold
        const double depth_similarity_tol  = 0.2;  // allow 1% difference to call depths "similar"
        ...
        for (size_t k = 0; k + 2 < m; ++k) {
...
#if 0
            Rprintf("\nk: %zu\n",k);
            Rprintf("e1.range_rel_depth: %f\t"
                    "e2.range_rel_depth: %f\t"
                    "e3.range_rel_depth: %f\n",
                    e1.range_rel_depth, e2.range_rel_depth, e3.range_rel_depth);
#endif
            
            // Small depths
            if (e1.range_rel_depth > small_depth_threshold ||
                e2.range_rel_depth > small_depth_threshold ||
                e3.range_rel_depth > small_depth_threshold)
                continue;

            // Check if either (e1,e2) or (e2,e3) have similar depths
...
#if 0
            Rprintf("similar_e1e2: %s\t"
                    "similar_e2e3: %s\n",
                    (similar_e1e2) ? "TRUE" : "FALSE",
                    (similar_e2e3) ? "TRUE" : "FALSE");
#endif

            if (!(similar_e1e2 || similar_e2e3))
                continue;

            // Now, repair between idx_1 and idx_3
            ...
            if (std::abs(y_start - y_end) < 1e-6) {
                ...
                if (idx_mid > idx_start && idx_mid < idx_end) {
                    ...
                    // Fallback to flat if midpoint out of bounds
                ...
                // Normal linear interpolation from start to end

                // debugging
                // Rprintf("In Normal linear interpolation from start to end block!!!\n");
