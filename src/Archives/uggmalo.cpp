...
// Potential optimizations for `uggmalo()`:

// 1. **Parallel Processing**: The bandwidth processing loop could be parallelized since each bandwidth is independent.

// 2. **Memory Management**:
//    - Preallocate vectors to their expected sizes
//    - Use move semantics more aggressively
//    - Consider using a memory pool for path objects

// 3. **Computation Reuse**:
//    - Cache kernel weights for reuse across similar distances
//    - Store and reuse path metrics that are independent of bandwidth

// 4. **Early Termination**:
//    - Add early stopping if error metrics plateau
//    - Skip bandwidths that are clearly suboptimal based on previous results

// Would you like me to elaborate on any of these optimizations or provide specific code examples for implementing them?
 ...
 * @param max_iterations Maximum number of iterations for linear regression fitting (default: 100)
 * @param ridge_lambda Ridge penalty parameter for linear regression (default: 0.0)
...
uggmalo_t uggmalo(
    ...
    result.graph_diameter = get_graph_diameter(adj_list, weight_list, start_vertex);
    ...
    uniform_grid_graph_t uniform_grid = create_uniform_grid_graph(
        adj_list,
        weight_list,
        grid_size,
        start_vertex,
    ...
    const size_t n_vertices = uniform_grid.n_original_vertices;
    ...
    // Creating an extended eigen_ulogit_t structure that contains also: mean_error, vertices and w_path components. The models will be sorted w/r to mean_error
    ...
    auto process_bw = [
        ...
        &verbose
            ...
            double bandwidth,
            const uniform_grid_graph_t& uniform_grid,
            const std::vector<double>& y,
        ...
        for (const auto& grid_vertex : uniform_grid.grid_vertices) {
        ...
        } // END OF for (const auto& grid_vertex : uniform_grid.grid_vertices)
...
SEXP S_uggmalo(
    ...
    std::vector<std::vector<int>> adj_list = Rgraph_to_vector(s_adj_list);
    std::vector<std::vector<double>> weight_list = Rweights_to_vector(s_weight_list);
...
#if 0
// Function to perform vertex-level permutation tests
...
vertex_wasserstein_perm_test_results_t vertex_wasserstein_perm_test(
    const std::vector<std::vector<double>>& bb_predictions,   // [n_bb][n_points]
    const std::vector<std::vector<double>>& null_predictions, // [n_perms][n_points]
    ...
    for (size_t i = 0; i < n_vertices; ++i) {
        ...
        for (size_t j = 0; j < n_tests; ++j) {
            vertex_null_values[j] = null_predictions[j][i];
        }
    }
    ...
    std::uniform_int_distribution<size_t> i_dist(0, n_vertices - 2);
    std::uniform_int_distribution<size_t> j_dist(1, n_vertices - 1);  // Initial range will be updated

    for (size_t k = 0; k < n_bootstraps; ++k) {
        size_t i = i_dist(gen);
        // Update the parameters of j_dist for this iteration
        j_dist.param(std::uniform_int_distribution<size_t>::param_type(i + 1, n_vertices - 1));
        size_t j = j_dist(gen);
        pairs.emplace_back(i, j);
    }

    size_t bootstrap_counter = 0;
    for (const auto& [i,j] : pairs) {
        C_wasserstein_distance_1D(
            null_values[i].data(),
            null_values[j].data(),
            &n_tests,
            results.null_distances.data() + bootstrap_counter++
            );
    }

    for (size_t i = 0; i < n_vertices; ++i) {
        // Get bootstrap distribution for this vertex
        vertex_bb_values.clear();
        for (size_t j = 0; j < n_tests; ++j) {
        ...
        size_t more_extreme = std::count_if(
            ...
            [&](double d) { return d >= results.wasserstein_distance; }
            );

        results.p_value = static_cast<double>(more_extreme) / n_bootstraps;

    }
}

/**
 * @brief Results structure for the Wasserstein-based permutation test
 */
struct wasserstein_test_results_t {
    double wasserstein_distance;      // Distance between bb and null distributions
    double p_value;                   // Computed p-value
    std::vector<double> null_distances; // Distribution of null Wasserstein distances
};

/**
 * @brief Performs Wasserstein-based permutation test comparing Bayesian bootstrap and null distributions
 */
wasserstein_test_results_t analyze_wasserstein_permutations(
    const std::vector<std::vector<double>>& bb_predictions,
    const std::vector<std::vector<double>>& null_predictions,
    size_t n_bootstraps = 1000
) {
    wasserstein_test_results_t results;
    const size_t n_vertices = bb_predictions[0].size();

    // Calculate average Wasserstein distance between bb and null distributions
    double total_distance = 0.0;
    size_t count = 0;

    // Using your efficient C implementation for Wasserstein distance
    std::vector<double> bb_sample(n_vertices);
    std::vector<double> null_sample(n_vertices);

    for (const auto& bb_pred : bb_predictions) {
        for (const auto& null_pred : null_predictions) {
    ...
    results.wasserstein_distance = total_distance / count;

    // Generate bootstrap null distribution
    std::random_device rd;
    std::mt19937 gen(rd());
    std::uniform_int_distribution<size_t> null_dist(0, null_predictions.size() - 1);

    results.null_distances.reserve(n_bootstraps);

    for (size_t b = 0; b < n_bootstraps; ++b) {
        // Sample two sets of predictions from null distribution
        const auto& null1 = null_predictions[null_dist(gen)];
        const auto& null2 = null_predictions[null_dist(gen)];

        double null_distance;
        int n = static_cast<int>(n_vertices);
        C_wasserstein_distance_1D(
            null1.data(),
            null2.data(),
        ...
        results.null_distances.push_back(null_distance);
    }
...
// Function to perform vertex-level permutation tests
struct vertex_test_results_t {
    std::vector<double> p_values;          // p-value for each vertex
    std::vector<double> effect_sizes;      // magnitude of effect at each vertex
    std::vector<bool> significant_vertices; // which vertices show significant effects
};

vertex_test_results_t analyze_vertex_permutations(
    const std::vector<std::vector<double>>& bb_predictions,    // [n_bb][n_points]
    const std::vector<std::vector<double>>& null_predictions,  // [n_perms][n_points]
    double alpha = 0.05  // significance level
    ) {
    const size_t n_points = bb_predictions[0].size();
    const size_t n_bb = bb_predictions.size();
    const size_t n_perms = null_predictions.size();

    vertex_test_results_t results;
    results.p_values.resize(n_points);
    results.effect_sizes.resize(n_points);
    results.significant_vertices.resize(n_points);

    // For each vertex
    for (size_t i = 0; i < n_points; ++i) {
        // Get bootstrap distribution for this vertex
        std::vector<double> vertex_bb_values(n_bb);
        for (size_t j = 0; j < n_bb; ++j) {
            vertex_bb_values[j] = std::abs(bb_predictions[j][i]);
        }

        // Get null distribution for this vertex
        std::vector<double> vertex_null_values(n_perms);
        for (size_t j = 0; j < n_perms; ++j) {
            vertex_null_values[j] = std::abs(null_predictions[j][i]);
        }

        // Count how many times bootstrap values exceed null values
        size_t count_exceeding = 0;
        size_t total_comparisons = 0;

        // Compare each bootstrap value against each null value
        for (double bb_val : vertex_bb_values) {
            for (double null_val : vertex_null_values) {
                if (bb_val > null_val) {
                    count_exceeding++;
                }
                total_comparisons++;
        ...
        // Calculate p-value for this vertex
        results.p_values[i] = 1.0 - static_cast<double>(count_exceeding) / total_comparisons;

        // Calculate effect size (e.g., mean difference between bootstrap and null)
        double mean_bb = std::accumulate(vertex_bb_values.begin(),
                                         vertex_bb_values.end(), 0.0) / n_bb;
        double mean_null = std::accumulate(vertex_null_values.begin(),
                                           vertex_null_values.end(), 0.0) / n_perms;
        results.effect_sizes[i] = mean_bb - mean_null;

        // Mark significant vertices
...
// 3. **Computation Reuse**:
//    - Cache kernel weights for reuse across similar distances <<--- this is more expensive than evaluation of the tricube kernel function
//    - Store and reuse path metrics that are independent of bandwidth <--- the only independent one is edge weights <<--- Added code to compute edge lengths

// 4. **Early Termination**:
//    - Add early stopping if error metrics plateau <<--- this may not be easy to determine
//    - Skip bandwidths that are clearly suboptimal based on previous results <<--- It seems to me tht to figure this out we need to process the bandwidth
