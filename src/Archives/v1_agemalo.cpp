...
agemalo_result_t agemalo(
    ...
    auto create_grid_vertex_models = [&](
		...
		// 5) Insert the mode into the queue
		...
		#if 0
		// find_graph_paths_within_radius() tests

		// 1. Save basic info (radius and grid vertex)
		{
			std::ofstream info_file(base_filename + "_info.csv");
			info_file << "parameter,value\n";
			info_file << "radius," << radius << "\n";
			info_file << "grid_vertex," << (grid_vertex + 1) << "\n";
			info_file << "graph_diameter," << result.graph_diameter << "\n";
			info_file << "total_paths," << shortest_paths.paths.size() << "\n";
			info_file << "reachable_vertices," << shortest_paths.reachable_vertices.size() << "\n";
		}

		// 2. Save all paths data in edge list format for igraph
		{
			std::ofstream paths_file(base_filename + "_paths.csv");
			paths_file << "path_id,from,to,weight\n";

			for (size_t path_idx = 0; path_idx < shortest_paths.paths.size(); ++path_idx) {
				const auto& path = shortest_paths.paths[path_idx];

				// Convert path to edge list format
				for (size_t i = 0; i < path.vertices.size() - 1; ++i) {
					size_t from = path.vertices[i];
					size_t to = path.vertices[i + 1];

					// Calculate edge weight (if needed, you might have to look this up)
					double edge_weight = 0.0;
					for (const auto& edge : grid_graph.adjacency_list[from]) {
						if (edge.vertex == to) {
					...
					paths_file << (path_idx + 1) << "," << (from + 1) << "," << (to + 1) << "," << edge_weight << "\n";
				}
			}
		}

		// 3. Save path endpoint data (useful for validation)
		{
			std::ofstream endpoints_file(base_filename + "_endpoints.csv");
			endpoints_file << "path_id,start_vertex,end_vertex,total_weight\n";

			for (size_t path_idx = 0; path_idx < shortest_paths.paths.size(); ++path_idx) {
				const auto& path = shortest_paths.paths[path_idx];
				if (!path.vertices.empty()) {
					size_t start = path.vertices.front();
					size_t end = path.vertices.back();
					endpoints_file << (path_idx + 1) << "," << (start + 1) << "," << (end + 1) << "," << path.total_weight << "\n";
				}
			}
		}

		// 4. Save reachable vertices
		{
			std::ofstream reachable_file(base_filename + "_reachable.csv");
			reachable_file << "vertex_id\n";
			for (const auto& vertex : shortest_paths.reachable_vertices) {
				reachable_file << (vertex + 1) << "\n";
			}
		}
		#endif
