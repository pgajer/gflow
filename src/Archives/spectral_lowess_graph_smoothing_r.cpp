...
SEXP S_spectral_lowess_graph_smoothing(
    ...
    switch (convergence_type_int) {
        ...
        default:
            UNPROTECT(nprot);
            Rf_error("Invalid convergence type: %d", convergence_type_int);
    }
    ...
    try {
        ...
        UNPROTECT(nprot);
        Rf_error("Error in spectral_lowess_graph_smoothing: %s", e.what());
    }
    
    // Create R objects for the results
    SEXP s_result = PROTECT(allocVector(VECSXP, 4)); nprot++;
    ...
    // Set names
    SEXP s_result_names = PROTECT(allocVector(STRSXP, 4)); nprot++;
