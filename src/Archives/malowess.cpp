 ...
 *
 * @return univariate_lm_t structure containing:
 *         Data components:
...
univariate_lm_t univariate_lm(const double* x,
                              const double* y,
    ...
    univariate_lm_t results;
    //results.x = x;
    results.w = w;
    results.x_min_index = x_min_index;
    results.x_max_index = x_max_index;
    ...
    for (int i = 0; i < n_points; ++i) {
        x_wmean += w[i] * x_working[i] / total_weight;
        y_wmean += w[i] * y[i] / total_weight;
    }

    // Center x_working around weighted mean for leverage calculation
    double sum_wx_squared = 0.0;
    for (int i = 0; i < n_points; ++i) {
        x_working[i] -= x_wmean;  // Now x is centered around its weighted mean
    ...
    for (int i = 0; i < n_points; i++) {
        results.predictions[i] = y_wmean + slope * (x[i] - x_wmean);
        if (y_binary) {
...
malowess_t malowess(...) {
    ...
    for (int k_index = 0, k = k_min; k <= k_max; k++, k_index++) {
        ...
        std::vector<std::vector<univariate_lm_t>> pt_models(n_points);
        for (int i = 0; i < n_points; i++) {
...
/**
 * @brief Implements the Model-Averaged LOWESS (MALOWESS) algorithm for robust local regression
 *
 * @details MALOWESS extends the traditional LOWESS algorithm by incorporating model averaging and
 ...
 *
 * @return malowess_t structure containing:
 ...
 *
 *
 * @see univariate_lm_t for the structure of individual linear models
 * @see initialize_kernel() for kernel function details
 */
malowess_t malowess(const std::vector<double>& x,
    ...
    memory_tracker_t mem_tracker("MALOWESS");

    if (verbose) {
        Rprintf("Starting MALOWESS computation\n");
    ...
    std::vector<std::pair<double, const univariate_lm_t*>> filtered_models;
    ...
    for (int k_index = 0, k = k_min; k <= k_max; k++, k_index++) {
        ...
        for (int i = 0; i < n_points; i++) {
            ...
            univariate_lm_t wlm_res = univariate_lm(x.data() + x_min_index,
                                                    y.data() + x_min_index,
                                                    w_window,
                                                    x_min_index,
                                                    x_max_index,
    ...
    if (verbose) {
        elapsed_time(total_ptm, "\nTotal MALOWESS computation time: ");
        Rprintf("Final ");
        mem_tracker.report();
    }
...
/**
 * @brief R interface for MALOWESS (Model-Averaged Locally Weighted Scatterplot Smoothing)
 *
...
SEXP S_malowess(SEXP s_x,
    ...
    malowess_t malowess_results = malowess(x,
                                           y,
                                           y_true,
                                           w,
                                           k_min,
    ...
    std::vector<int> k_values(malowess_results.k_mean_sm_errors.size());
    for (int k_index = 0, k = k_min; k <= k_max; k++, k_index++)
        k_values[k_index] = k;
    ...
    INTEGER(s_opt_sm_k)[0] = malowess_results.opt_sm_k;
    SET_VECTOR_ELT(result, 1, s_opt_sm_k);

    SEXP s_opt_sm_k_idx = PROTECT(allocVector(INTSXP, 1)); n_protected++;
    INTEGER(s_opt_sm_k_idx)[0] = malowess_results.opt_sm_k_idx;
    SET_VECTOR_ELT(result, 2, s_opt_sm_k_idx);

    // opt_ma_k
    SEXP s_opt_ma_k = PROTECT(allocVector(INTSXP, 1)); n_protected++;
    INTEGER(s_opt_ma_k)[0] = malowess_results.opt_ma_k;
    SET_VECTOR_ELT(result, 3, s_opt_ma_k);

    SEXP s_opt_ma_k_idx = PROTECT(allocVector(INTSXP, 1)); n_protected++;
    INTEGER(s_opt_ma_k_idx)[0] = malowess_results.opt_ma_k_idx;
    SET_VECTOR_ELT(result, 4, s_opt_ma_k_idx);

    // k_mean_sm_errors
    SET_VECTOR_ELT(result, 5, PROTECT(convert_vector_double_to_R(malowess_results.k_mean_sm_errors))); n_protected++;
    // k_mean_ma_errors
    SET_VECTOR_ELT(result, 6, PROTECT(convert_vector_double_to_R(malowess_results.k_mean_ma_errors))); n_protected++;

    // true errors
    if (y_true_exists) {
        SET_VECTOR_ELT(result, 7, PROTECT(convert_vector_double_to_R(malowess_results.k_mean_sm_true_errors))); n_protected++;
        SET_VECTOR_ELT(result, 8, PROTECT(convert_vector_double_to_R(malowess_results.k_mean_ma_true_errors))); n_protected++;
    } else {
    ...
    // predictions
    SET_VECTOR_ELT(result, 9, PROTECT(convert_vector_double_to_R(malowess_results.sm_predictions))); n_protected++;
    SET_VECTOR_ELT(result, 10, PROTECT(convert_vector_double_to_R(malowess_results.ma_predictions))); n_protected++;

    // k_predictions
    SET_VECTOR_ELT(result, 11, PROTECT(convert_vector_vector_double_to_R(malowess_results.k_sm_predictions))); n_protected++;
    SET_VECTOR_ELT(result, 12, PROTECT(convert_vector_vector_double_to_R(malowess_results.k_ma_predictions))); n_protected++;
