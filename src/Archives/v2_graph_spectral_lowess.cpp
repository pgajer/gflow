...
#define DEBUG__graph_spectral_lowess 0
	...
	if (verbose) {
...
#if DEBUG__graph_spectral_lowess
		...
		{
			// debugging/testing - saving to file eigenvectors
			...
			std::string eigenvectors_file_name = debug_dir + "eigenvectors.csv";
			std::ofstream eigenvectors_file(eigenvectors_file_name);
			if (eigenvectors_file.is_open()) {
				// Get the eigenvectors matrix
				Eigen::MatrixXd evectors = eigs.eigenvectors();

				// Write header row with column names
				eigenvectors_file << "vertex";
				for (int j = 0; j < std::min(6, (int)evectors.cols()); j++) {
					eigenvectors_file << ",eigenvector" << (j+1);
				}
				eigenvectors_file << "\n";

				// Write each row (vertex) with its corresponding eigenvector entries
				for (int i = 0; i < evectors.rows(); i++) {
					eigenvectors_file << (i+1);  // 1-indexed for R compatibility
					for (int j = 0; j < std::min(6, (int)evectors.cols()); j++) {
						eigenvectors_file << "," << evectors(i, j);
					}
					eigenvectors_file << "\n";
				}

				eigenvectors_file.close();
				Rprintf("\nEigenvectors were written to file: %s\n", eigenvectors_file_name.c_str());
			} else {
				REPORT_WARNING("Failed to open file for writing eigenvectors: %s\n",
							   eigenvectors_file_name.c_str());
			}
		}
#endif
	...
	std::for_each(std::execution::seq, vertices.begin(), vertices.end(),
				  [&](size_t vertex) {
					  try {
						  ...
						  print_umap(reachable_vertices_map, "reachable_vertices_map");
#endif
...
#if DEBUG__graph_spectral_lowess
							  Rprintf("\nbw_idx: %zu\n", bw_idx);
							  Rprintf("local_vertex_map.size(): %zu\n", local_vertex_map.size());
							  print_map(local_vertex_map, "local_vertex_map");
#endif
...
#if DEBUG__graph_spectral_lowess
							  {
								  // debugging/testing - saving to file embedding
								  std::string debug_dir = "/Users/pgajer/current_projects/msr2/debugging_data/";
								  if (!std::filesystem::exists(debug_dir)) {
									  if (!std::filesystem::create_directories(debug_dir)) {
										  REPORT_ERROR("ERROR: Failed to create debug directory: %s\n", debug_dir.c_str());
								  ...
								  std::string embedding_file_name = debug_dir + "embedding_vertex_" + std::to_string(vertex) + "_bw_" + std::to_string(bw_idx) + ".csv";
								  std::ofstream embedding_file(embedding_file_name);
								  if (embedding_file.is_open()) {
									  // Write header row
									  embedding_file << "vertex";
									  for (int j = 0; j < embedding.cols(); j++) {
										  embedding_file << ",dim" << j;
									  }
									  embedding_file << "\n";

									  // Write each row of the embedding with the corresponding vertex and distance
									  int row_idx = 0;
									  for (const auto& [v_idx, distance] : local_vertex_map) {
										  embedding_file << (v_idx + 1);  // 1-indexed vertex for R compatibility

										  // Write all embedding coordinates for this vertex
										  for (int j = 0; j < embedding.cols(); j++) {
											  embedding_file << "," << embedding(row_idx, j);
										  }
										  embedding_file << "\n";
										  row_idx++;
									  }
									  ...
									  if (verbose) {
										  Rprintf("Spectral embedding for vertex %zu (bw=%.4f) written to: %s\n",
												  vertex, current_bw, embedding_file_name.c_str());
									  }

								  } else {
									  REPORT_WARNING("Failed to open file for writing embedding: %s\n",
...
#if DEBUG__graph_spectral_lowess
							  {  // debugging/testing - saving model data
								  std::string debug_dir = "/Users/pgajer/current_projects/msr2/debugging_data/";
								  std::string model_file_name = debug_dir + "model_vertex_" + std::to_string(vertex) + "_bw_" + std::to_string(bw_idx) + ".csv";
								  model.write_model_data_to_csv(
									  embedding,
									  y,
									  local_vertex_map,
									  model_file_name);
...
#if DEBUG__graph_spectral_lowess
						  {
							  // debugging/testing - saving bandwidth vs error data
							  std::string debug_dir = "/Users/pgajer/current_projects/msr2/debugging_data/";
							  if (!std::filesystem::exists(debug_dir)) {
								  if (!std::filesystem::create_directories(debug_dir)) {
									  REPORT_ERROR("ERROR: Failed to create debug directory: %s\n", debug_dir.c_str());
							  ...
							  std::string bw_errors_filename = debug_dir + "bandwidth_errors_vertex_" + std::to_string(vertex) + ".csv";
							  std::ofstream bw_errors_file(bw_errors_filename);

							  if (bw_errors_file.is_open()) {
								  // Write header
								  bw_errors_file << "bandwidth,error\n";

								  // Write data rows
								  for (size_t i = 0; i < candidate_bws.size(); i++) {
									  if (i < bandwidth_errors.size()) {
										  bw_errors_file << candidate_bws[i] << "," << bandwidth_errors[i] << "\n";
									  }
								  }

								  bw_errors_file.close();
								  if (verbose) {
									  Rprintf("Bandwidth vs. error data for vertex %zu written to: %s\n",
											  vertex, bw_errors_filename.c_str());
								  }
							  } else {
								  REPORT_WARNING("Failed to open file for writing bandwidth error data: %s\n",
												 bw_errors_filename.c_str());
							  }
						  }
#endif
