...
gradient_basin_t set_wgraph_t::compute_basin_of_attraction(
    ...
    // BFS to find the basin and build all_predecessors for shortest monotone paths
    std::vector<int> hop_distance(adjacency_list.size(), -1);
    std::queue<size_t> q;
    ...
    result.hop_dist_map[vertex] = 0;
    hop_distance[vertex] = 0;
    result.all_predecessors[vertex] = std::vector<size_t>();  // Origin has no predecessors
    q.push(vertex);

    size_t max_hop_distance = 0;

    // BFS with monotonicity constraints, tracking all optimal predecessors
    while (!q.empty()) {
        size_t u = q.front();
        q.pop();
        ...
        for (const auto& edge : adjacency_list[u]) {
            ...
            if (hop_distance[v] == -1) {
                // First time visiting v - this is a shortest path
                hop_distance[v] = hop_distance[u] + 1;
                result.hop_dist_map[v] = static_cast<size_t>(hop_distance[v]);
                result.all_predecessors[v] = std::vector<size_t>();
                result.all_predecessors[v].push_back(u);
                ...
                max_hop_distance = std::max(max_hop_distance,
                                           static_cast<size_t>(hop_distance[v]));
            } else if (hop_distance[v] == hop_distance[u] + 1) {
                // Already visited at same hop distance - this is an alternative shortest path
                result.all_predecessors[v].push_back(u);
            }
            // If hop_distance[v] < hop_distance[u] + 1, this edge is not on a shortest path, ignore
        }
    }

    result.hop_idx = max_hop_distance;

    // Build successor map from all_predecessors
    ...
    for (const auto& [v, preds] : result.all_predecessors) {
        for (size_t pred_v : preds) {
            successors[pred_v].push_back(v);
        }
    }

    // Identify s-terminals (structural): vertices with no successors
    std::vector<size_t> s_terminals;
    for (const auto& [v, succ_list] : successors) {
        if (v == vertex) continue;  // Skip origin
        if (succ_list.empty()) {
            s_terminals.push_back(v);
        }
    }

    // Use s-terminals for cell structure
    result.terminal_extrema = s_terminals;

    // Optional: Compute m-terminals for comparison (diagnostic purposes)
    if (with_trajectories) {
        std::vector<size_t> m_terminals;
        for (const auto& [u, hop] : result.hop_dist_map) {
        ...
        Rprintf("Basin size: %zu vertices\n", result.hop_dist_map.size());
        Rprintf("Max hop distance: %zu\n", max_hop_distance);
        Rprintf("s-terminals (no successors): %zu\n", s_terminals.size());
        Rprintf("m-terminals (local min in basin): %zu\n", m_terminals.size());
        Rprintf("===================================\n\n");
    }
    ...
    // Compute trajectories if requested
    if (with_trajectories) {
        // Enumerate all shortest monotone paths for each s-terminal
        for (size_t terminal : result.terminal_extrema) {
            trajectory_set_t traj_set;
            traj_set.terminal_vertex = terminal;

            // Find all shortest paths from terminal to origin
            std::vector<size_t> empty_path;
            enumerate_all_paths(terminal, vertex, result.all_predecessors,
                              empty_path, traj_set.trajectories);
        ...
        // DIAGNOSTIC: Verify trajectory coverage
        std::unordered_set<size_t> vertices_in_trajectories;
        for (const auto& traj_set : result.trajectory_sets) {
            for (const auto& traj : traj_set.trajectories) {
                for (size_t v : traj) {
        ...
        if (uncovered > 0) {
            Rprintf("WARNING: Incomplete trajectory coverage!\n");

            // Analyze uncovered vertices
            size_t sample_count = 0;
            for (const auto& [v, hop] : result.hop_dist_map) {
                if (v == vertex) continue;
                if (vertices_in_trajectories.find(v) != vertices_in_trajectories.end()) {
                    continue;
                }

                if (sample_count < 5) {
                    Rprintf("  Uncovered vertex %zu (hop=%zu):\n", v, hop);

                    auto pred_it = result.all_predecessors.find(v);
                    size_t n_preds = (pred_it != result.all_predecessors.end()) ?
                                    pred_it->second.size() : 0;
                    Rprintf("    Predecessors: %zu\n", n_preds);
                    Rprintf("    Successors: %zu\n", successors[v].size());

                    // Check if it's a terminal
                    bool is_terminal = (successors[v].empty() && v != vertex);
                    Rprintf("    Is s-terminal: %s\n", is_terminal ? "YES" : "NO");
