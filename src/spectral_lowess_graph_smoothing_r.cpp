...
SEXP S_spectral_lowess_graph_smoothing(
    ...
    bool compute_errors = asLogical(s_compute_errors);
    bool compute_scales = asLogical(s_compute_scales);
    ...
    SEXP s_result = PROTECT(allocVector(VECSXP, 7)); nprot++;  // Increased from 5 to 7 for the new fields
    ...
    // Set names
    SEXP s_result_names = PROTECT(allocVector(STRSXP, 7)); nprot++;  // Increased from 5 to 7
