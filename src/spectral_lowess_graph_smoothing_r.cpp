...
/**
 * @brief R interface for the spectral_lowess_graph_smoothing function
 * 
 * This function serves as the interface between R and the C++ implementation of
 * spectral_lowess_graph_smoothing. It converts R objects to C++ types, calls the
 * C++ function, and converts the results back to R objects.
 */
SEXP S_spectral_lowess_graph_smoothing(
    SEXP s_adj_list,
    SEXP s_weight_list,
    ...
    bool compute_errors = Rf_asLogical(s_compute_errors);
    bool compute_scales = Rf_asLogical(s_compute_scales);
    ...
    try {
        result = spectral_lowess_graph_smoothing(
            initial_graph,
            X,
            max_iterations,
        ...
        REPORT_ERROR("Error in spectral_lowess_graph_smoothing: %s", e.what());
    }

    // Create R objects for the results
    SEXP s_result = PROTECT(Rf_allocVector(VECSXP, 7));   // Increased from 5 to 7 for the new fields

    // Set names
    {
        SEXP s_result_names = PROTECT(Rf_allocVector(STRSXP, 7));   // Increased from 5 to 7
