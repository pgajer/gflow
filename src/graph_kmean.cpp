 ...
 */
extern "C" SEXP S_univariate_gkmm(SEXP s_x,
                                  SEXP s_y,
                                  SEXP s_y_true,
                                  SEXP s_use_median,
                                  SEXP s_h_min,
  ...
  std::vector<double> x, y, y_true;
  {
    ...
    if (syt != R_NilValue && TYPEOF(syt) != REALSXP)
      REPROTECT(syt = Rf_coerceVector(syt, REALSXP), pitz);

    const int nx = LENGTH(sx);
  ...
  // ---- parent result (protect early to silence rchk) ----
  ...
  // ---- graphs list (VECSXP of length K) + names ----
  const long long Kll = (long long)h_max - (long long)h_min + 1LL;
  if (Kll < 0 || Kll > (long long)INT_MAX) { UNPROTECT(1); Rf_error("too large"); }
  const int K = (int)Kll;

  SEXP s_hHN_graphs = PROTECT(Rf_allocVector(VECSXP, K));
  SEXP h_names      = PROTECT(Rf_allocVector(STRSXP, K));

  for (int i = 0; i < K; ++i) {
    // h_graph: list(adj_list, edge_lengths)
    SEXP h_graph = PROTECT(Rf_allocVector(VECSXP, 2));

    // adj_list: replace convert_vector_vector_int_to_R
    {
      const size_t n = cpp_results.graphs[(size_t)i].first.size();
      if (n > (size_t)INT_MAX) { UNPROTECT(1); UNPROTECT(2); UNPROTECT(1); Rf_error("too large"); }
      SEXP out = PROTECT(Rf_allocVector(VECSXP, (int)n));
      for (int ii = 0; ii < (int)n; ++ii) {
        const auto& vi = cpp_results.graphs[(size_t)i].first[(size_t)ii];
        const size_t m = vi.size();
        if (m > (size_t)INT_MAX) { UNPROTECT(1); UNPROTECT(1); UNPROTECT(2); UNPROTECT(1); Rf_error("too large"); }
        SEXP v = PROTECT(Rf_allocVector(INTSXP, (int)m));
        int* pv = INTEGER(v);
        for (int j = 0; j < (int)m; ++j) pv[j] = vi[(size_t)j];
        SET_VECTOR_ELT(out, ii, v);
        UNPROTECT(1);
      }
      SET_VECTOR_ELT(h_graph, 0, out);
      UNPROTECT(1); // out
    }

    // edge_lengths: replace convert_vector_vector_double_to_R
    {
      const size_t n = cpp_results.graphs[(size_t)i].second.size();
      if (n > (size_t)INT_MAX) { UNPROTECT(1); UNPROTECT(2); UNPROTECT(1); Rf_error("too large"); }
      SEXP out = PROTECT(Rf_allocVector(VECSXP, (int)n));
      for (int ii = 0; ii < (int)n; ++ii) {
        const auto& vi = cpp_results.graphs[(size_t)i].second[(size_t)ii];
        const size_t m = vi.size();
        if (m > (size_t)INT_MAX) { UNPROTECT(1); UNPROTECT(1); UNPROTECT(2); UNPROTECT(1); Rf_error("too large"); }
        SEXP v = PROTECT(Rf_allocVector(REALSXP, (int)m));
        double* pv = REAL(v);
        for (int j = 0; j < (int)m; ++j) pv[j] = vi[(size_t)j];
        SET_VECTOR_ELT(out, ii, v);
        UNPROTECT(1);
      }
      SET_VECTOR_ELT(h_graph, 1, out);
      UNPROTECT(1); // out
    }

    // names for h_graph
    {
      SEXP gn = PROTECT(Rf_allocVector(STRSXP, 2));
    ...
    char h_label[32];
    std::snprintf(h_label, sizeof(h_label), "h_%d", h_min + i);
    SET_STRING_ELT(h_names, i, Rf_mkChar(h_label));
  }

  Rf_setAttrib(s_hHN_graphs, R_NamesSymbol, h_names);
  UNPROTECT(1); // h_names
  SET_VECTOR_ELT(r_result, 1, s_hHN_graphs);
  UNPROTECT(1); // s_hHN_graphs

  // ---- 0: h_values  (replace convert_vector_int_to_R) ----
  {
    const size_t n = cpp_results.h_values.size();
    if (n > (size_t)INT_MAX) { UNPROTECT(1); Rf_error("too large"); }
    SEXP hv = PROTECT(Rf_allocVector(INTSXP, (int)n));
    int* p = INTEGER(hv);
    for (int i = 0; i < (int)n; ++i) p[i] = cpp_results.h_values[(size_t)i];
    SET_VECTOR_ELT(r_result, 0, hv);
    UNPROTECT(1);
  }

  // ---- 2: h_cv_errors or NULL (replace convert_vector_double_to_R) ----
  {
    if (!cpp_results.cv_errors.empty() && !x.empty()) {
      const size_t n = cpp_results.cv_errors.size();
      if (n > (size_t)INT_MAX) { UNPROTECT(1); Rf_error("too large"); }
      SEXP ce = PROTECT(Rf_allocVector(REALSXP, (int)n));
      double* p = REAL(ce);
      for (int i = 0; i < (int)n; ++i) p[i] = cpp_results.cv_errors[(size_t)i];
      SET_VECTOR_ELT(r_result, 2, ce);
      UNPROTECT(1);
    } else {
  ...
  // ---- 3: opt_h ----
  {
    SEXP opt_h = PROTECT(Rf_ScalarReal(cpp_results.opt_h));
    SET_VECTOR_ELT(r_result, 3, opt_h);
    UNPROTECT(1);
  }

  // ---- 4: opt_graph_adj_list (inline) ----
  {
    const auto& X = cpp_results.opt_h_graph.first;
    const size_t n = X.size();
    if (n > (size_t)INT_MAX) { UNPROTECT(1); Rf_error("too large"); }
    SEXP out = PROTECT(Rf_allocVector(VECSXP, (int)n));
    for (int i = 0; i < (int)n; ++i) {
      const auto& vi = X[(size_t)i];
      const size_t m = vi.size();
      if (m > (size_t)INT_MAX) { UNPROTECT(1); UNPROTECT(1); Rf_error("too large"); }
      SEXP v = PROTECT(Rf_allocVector(INTSXP, (int)m));
      int* pv = INTEGER(v);
      for (int j = 0; j < (int)m; ++j) pv[j] = vi[(size_t)j];
    ...
    SET_VECTOR_ELT(r_result, 4, out);
    UNPROTECT(1);
  }

  // ---- 5: opt_graph_edge_lengths (inline) ----
  {
    const auto& X = cpp_results.opt_h_graph.second;
    const size_t n = X.size();
    if (n > (size_t)INT_MAX) { UNPROTECT(1); Rf_error("too large"); }
    SEXP out = PROTECT(Rf_allocVector(VECSXP, (int)n));
    for (int i = 0; i < (int)n; ++i) {
    ...
    SET_VECTOR_ELT(r_result, 5, out);
    UNPROTECT(1);
  }

  // ---- 6: predictions (condEy) inline ----
  {
    const size_t n = cpp_results.condEy.size();
    if (n > (size_t)INT_MAX) { UNPROTECT(1); Rf_error("too large"); }
    SEXP pr = PROTECT(Rf_allocVector(REALSXP, (int)n));
    double* p = REAL(pr);
    for (int i = 0; i < (int)n; ++i) p[i] = cpp_results.condEy[(size_t)i];
  ...
  // ---- 7–9: bb + CIs (inline) ----
  {
    if (!cpp_results.bb_condEy.empty()) {
      // 7) bb_condEy
      {
        const size_t n = cpp_results.bb_condEy.size();
        if (n > (size_t)INT_MAX) { UNPROTECT(1); Rf_error("too large"); }
        SEXP bb = PROTECT(Rf_allocVector(REALSXP, (int)n));
        double* p = REAL(bb);
        for (int i = 0; i < (int)n; ++i) p[i] = cpp_results.bb_condEy[(size_t)i];
        SET_VECTOR_ELT(r_result, 7, bb);
        UNPROTECT(1);
      }
      // 8) L
      {
        const size_t n = cpp_results.cri_L.size();
        if (n > (size_t)INT_MAX) { UNPROTECT(1); Rf_error("too large"); }
        SEXP L = PROTECT(Rf_allocVector(REALSXP, (int)n));
        double* p = REAL(L);
        for (int i = 0; i < (int)n; ++i) p[i] = cpp_results.cri_L[(size_t)i];
        SET_VECTOR_ELT(r_result, 8, L);
        UNPROTECT(1);
      }
      // 9) U
      {
        const size_t n = cpp_results.cri_U.size();
        if (n > (size_t)INT_MAX) { UNPROTECT(1); Rf_error("too large"); }
        SEXP U = PROTECT(Rf_allocVector(REALSXP, (int)n));
        double* p = REAL(U);
        for (int i = 0; i < (int)n; ++i) p[i] = cpp_results.cri_U[(size_t)i];
        SET_VECTOR_ELT(r_result, 9, U);
        UNPROTECT(1);
      }
    } else {
  ...
  // ---- 10: true_error (mean) or NULL ----
  {
    if (!cpp_results.true_errors.empty()) {
      ...
      SEXP te = PROTECT(Rf_ScalarReal(mean_true_error));
      SET_VECTOR_ELT(r_result, 10, te);
      UNPROTECT(1);
    } else {
