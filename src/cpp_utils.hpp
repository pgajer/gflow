...
void print_row_major_2D_array(const T* matrix,
    ...
    std::cout << std::string(static_cast<size_t>(row_index_width + 1), ' ');
    for (int j = 0; j < n_cols; ++j) {
        size_t idx = static_cast<size_t>(j);
        std::cout << std::setw(col_widths[idx]) << std::right << "[," << (j + base) << "]";
    ...
    for (size_t i = 0; i < n && i < static_cast<size_t>(n_rows); ++i) {
        if (with_row_index) {
            std::cout << "[" << std::setw(static_cast<size_t>(row_index_width - 1)) << std::right << (i + base) << ",]";
        }
        for (int j = 0; j < n_cols; ++j) {
            if (j == 0) {
                std::cout << std::setw(col_widths[static_cast<size_t>(j)] + 1) << std::right;
            } else {
                std::cout << std::setw(col_widths[static_cast<size_t>(j)] + 2) << std::right;
            }
            ...
            if constexpr (std::is_same_v<T, std::pair<int, int>>) {
                ...
                std::cout << (matrix[i * n_cols + j] + base);
            } else if constexpr (std::is_floating_point_v<T>) {
                std::cout << std::setprecision(7) << std::fixed << matrix[i * n_cols + j];
            } else {
...
void print_column_major_2D_array(const T* matrix,
    ...
    std::cout << "     ";
    for (int j = 0; j < n_cols; ++j) {
        std::cout << "[," << std::setw(col_widths[j]) << j + 1 << "]";
        if (j < n_cols - 1) std::cout << " ";
    }
    std::cout << std::endl;
    ...
    for (size_t i = 0; i < n && i < static_cast<size_t>(n_rows); ++i) {
        ...
        for (int j = 0; j < n_cols; ++j) {
            if constexpr (std::is_same_v<T, std::pair<int, int>>) {
                ...
                std::cout << std::setw(col_widths[j]) << (matrix[j * n_rows + i] + base);
            } else if constexpr (std::is_floating_point_v<T>) {
                std::cout << std::setw(col_widths[j]) << std::setprecision(7) << std::fixed << matrix[j * n_rows + i];
            } else {
                std::cout << std::setw(col_widths[j]) << matrix[j * n_rows + i];
            }
            if (j < n_cols - 1) std::cout << " ";
        }
        std::cout << std::endl;
    ...
    for (size_t i = 0; i < n && i < static_cast<size_t>(n_rows); ++i) {
        ...
        for (int j = 0; j < n_cols; ++j) {
            if constexpr (std::is_same_v<T, std::pair<int, int>>) {
                ...
                std::cout << std::setw(8) << std::setprecision(4) << matrix[j * n_rows + i] << " ";
            }
        }
        std::cout << std::endl;
    }
