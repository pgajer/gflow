/**
 * @brief Hardened rchk-safe drop-in of S_parameterize_circular_graph
 *
 * Changes vs. provided version:
 * - Keep `result` and `names` protected until the tail; end with UNPROTECT(2).
 * - Maintain container-first assembly with fixed-count UNPROTECTs.
 * - Defensive scalar extraction via Rf_as* helpers.
 */
#include <vector>
#include <R.h>
...
SEXP S_parameterize_circular_graph(
    ...
    // Convert inputs using trusted helpers
    ...
    // Defensive scalar extraction (Rf_as*)
    int start_vertex   = Rf_asInteger(start_vertex_sexp) - 1; // 0-based
    ...
    if (start_vertex < 0 || start_vertex >= n_vertices) {
        Rf_error("S_parameterize_circular_graph: 'start_vertex' is out of bounds");
    }
    ...
    // Assemble result (container-first). Keep `result` and `names` protected until tail.
    ...
    {
        const R_xlen_t m = (R_xlen_t) param.ordering.size();
        SEXP ordering = PROTECT(Rf_allocVector(INTSXP, m));
        int* ord_ptr = INTEGER(ordering);
        for (R_xlen_t i = 0; i < m; ++i) {
            ord_ptr[i] = param.ordering[(size_t)i] + 1;
        }
    ...
    // names — keep protected until the end
