// graph_kernel_smoother_r_hardened.cpp — rchk-hardened drop-in
//
// Hardenings vs provided file:
// - Coercion blocks for y & weights with PROTECT_WITH_INDEX/REPROTECT; copy to STL then UNPROTECT immediately.
...
// - Validate ny==nw and ny matches graph size (adj_list.size()).
// - Result assembly keeps only `result` + `names` protected to tail → final UNPROTECT(2).
// - Long-vector safety (R_xlen_t for sizes; explicit casts when copying).
//
...
SEXP S_graph_kernel_smoother(
    ...
    // --- Coercion blocks for y and weights ---
    std::vector<double> y;
    ...
    }
    {
        ...
        nw = XLENGTH(w_real);
        if (nw <= 0) { UNPROTECT(1); Rf_error("S_graph_kernel_smoother(): 'weights' must be a non-empty numeric vector."); }
        weights.assign(REAL(w_real), REAL(w_real) + (size_t)nw);
        UNPROTECT(1); // w_real
    }

    if (ny != nw) {
        Rf_error("S_graph_kernel_smoother(): 'y' and 'weights' must have the same length.");
    }
    ...
    if ((size_t)ny != adj_list.size()) {
        Rf_error("S_graph_kernel_smoother(): length(y) must match number of graph vertices (%llu).", (unsigned long long)adj_list.size());
    }
    ...
    // 0: smoothed_values
    {
        SEXP s = PROTECT(convert_vector_double_to_R(result_data.smoothed_values));
        SET_VECTOR_ELT(result, 0, s);
        UNPROTECT(1);
    }
    // 1: kernel_weights
    {
        SEXP s = PROTECT(convert_vector_double_to_R(result_data.kernel_weights));
        SET_VECTOR_ELT(result, 1, s);
        UNPROTECT(1);
    }
    // 2: bandwidth
    {
        SEXP s = PROTECT(Rf_ScalarReal(result_data.bandwidth));
        SET_VECTOR_ELT(result, 2, s);
        UNPROTECT(1);
    }
    // 3: effective_df
    {
        SEXP s = PROTECT(Rf_ScalarReal(result_data.effective_df));
        SET_VECTOR_ELT(result, 3, s);
        UNPROTECT(1);
    }
    // 4: influence_diagonal
    {
        SEXP s = PROTECT(convert_vector_double_to_R(result_data.influence_diagonal));
        SET_VECTOR_ELT(result, 4, s);
        UNPROTECT(1);
    }
    // 5: kernel_type
    {
        SEXP s = PROTECT(Rf_ScalarInteger(result_data.kernel_type));
        SET_VECTOR_ELT(result, 5, s);
        UNPROTECT(1);
    }
    // 6: converged
    {
        SEXP s = PROTECT(Rf_ScalarLogical(result_data.converged ? TRUE : FALSE));
