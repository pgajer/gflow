// geodesic_stats_r_hardened.cpp — rchk-hardened drop-in for S_compute_geodesic_stats
//
// Hardenings applied vs. provided file:
// - Keep only `result` and `result_names` protected until the tail → final UNPROTECT(2).
// - All other allocations are protected locally and unprotected immediately after insertion.
// - Defensive scalar handling via Rf_as* with NA/range checks (min/max radii, steps, packing params, etc.).
// - Long-vector safety for sizes (R_xlen_t casts).
// - Dimnames created per-matrix in a local protected scope and released immediately.
//
// Assumptions: helper converters and core algorithms exist with the same signatures as in the original.
...
// Core structures
struct overlap_statistics_t {
    double min, p05, p25, median, p75, p95, max;
};

struct geodesic_stats_t {
    std::vector<double> radii;
    std::vector<std::unordered_map<size_t, int>> geodesic_rays;
    std::vector<std::unordered_map<size_t, int>> composite_geodesics;
    std::vector<std::unordered_map<size_t, overlap_statistics_t>> paths_overlap;
    std::vector<std::vector<double>> radius_overlaps;
};

class uniform_grid_graph_t {
public:
    std::set<size_t> grid_vertices;
    double graph_diameter;

    uniform_grid_graph_t() : graph_diameter(0.0) {}
...
// Core routines (provided elsewhere)
