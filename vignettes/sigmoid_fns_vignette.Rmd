...
apply.sigmoid <- function(x, alpha = 1.0, type = c("tanh", "arctan", "algebraic")) {
  type <- match.arg(type)
  stopifnot(is.numeric(x), is.numeric(alpha), length(alpha) == 1, alpha > 0)

  switch(type,
         tanh = tanh(alpha * x),
         arctan = (2 / pi) * atan(alpha * x),
         algebraic = x / sqrt((1 / alpha^2) + x^2))
}
```
...
The helper `apply.sigmoid()` used in this vignette provides access to all three transformations. 
