...
SEXP S_compute_graph_analysis_sequence(SEXP s_adj_list,
  ...
  const int nres = (int)results.size();
  SEXP r_results_list = PROTECT(Rf_allocVector(VECSXP, nres)); // [1]

  for (int i = 0; i < nres; ++i) {
    const auto& ms = results[(size_t)i];

    SEXP ms_list = PROTECT(Rf_allocVector(VECSXP, 12));     // [2]

    // child = PROTECT(convert_*()); SET_VECTOR_ELT; UNPROTECT(1)
    {
      SEXP t;

      t = PROTECT(convert_set_to_R(ms.local_maxima));
      SET_VECTOR_ELT(ms_list, 0, t); UNPROTECT(1);

      t = PROTECT(convert_set_to_R(ms.local_minima));
      SET_VECTOR_ELT(ms_list, 1, t); UNPROTECT(1);

      t = PROTECT(convert_map_set_to_R(ms.lmax_to_lmin));
      SET_VECTOR_ELT(ms_list, 2, t); UNPROTECT(1);

      t = PROTECT(convert_map_set_to_R(ms.lmin_to_lmax));
      SET_VECTOR_ELT(ms_list, 3, t); UNPROTECT(1);

      t = PROTECT(convert_procells_to_R(ms.procells));
      SET_VECTOR_ELT(ms_list, 4, t); UNPROTECT(1);

      t = PROTECT(convert_map_vector_set_to_R(ms.cells));
      SET_VECTOR_ELT(ms_list, 5, t); UNPROTECT(1);
      ...
      t = PROTECT(convert_map_vector_to_R(ms.shortest_paths));
      SET_VECTOR_ELT(ms_list,10, t); UNPROTECT(1);

      t = PROTECT(convert_vector_double_to_R(ms.Ey));
      SET_VECTOR_ELT(ms_list,11, t); UNPROTECT(1);
    }

    {
      SEXP names = PROTECT(Rf_allocVector(STRSXP, 12));   // [3]
