...
#' This is a helper function used internally by \code{graph.spectrum}.
#'
#' @param adj.list A list of integer vectors representing the adjacency list.
#'   Each element corresponds to a vertex and contains the indices of its
#'   adjacent vertices.
#'
#' @return A symmetric binary matrix where entry (i,j) is 1 if vertices i and j
#'   are connected, and 0 otherwise.
#'
#' @details
#' This function assumes an undirected graph and creates a symmetric adjacency
#' matrix. It handles self-loops by setting diagonal entries to 0.
#'
#' @examples
#' # Simple triangle graph
#' adj_list <- list(c(2, 3), c(1, 3), c(1, 2))
#' adj_matrix <- convert.adjacency.list.to.adjacency.matrix(adj_list)
#'
#' @keywords internal
#' @noRd
    ...
    for (i in seq_len(n)) {
        ...
        if (length(neighbors) > 0) {
            ...
            if (any(neighbors < 1) || any(neighbors > n)) {
                stop("Invalid vertex indices in adjacency list")
            }
            # Avoid self-loops
            neighbors <- neighbors[neighbors != i]
            if (length(neighbors) > 0) {
                adj.matrix[i, neighbors] <- 1
                adj.matrix[neighbors, i] <- 1  # Ensure symmetry
            }
        }
    }
