...
#'
#' @param X A numeric matrix where rows represent vertices and columns represent
#'   features to be smoothed.
#' @param graph A list of integer vectors representing the adjacency structure
#'   (1-based indexing).
#' @param edge.lengths A list of numeric vectors containing edge lengths.
#' @param weights An optional numeric vector of vertex weights. If \code{NULL},
#'   all vertices are weighted equally.
#' @param n.time.steps An integer specifying the number of diffusion steps.
#'   Default is 100.
#' @param step.factor A numeric value between 0 and 1 for step size. Default is 0.5.
#' @param normalize An integer (0, 1, or 2) for normalization method. Default is 0.
#' @param imputation.method Either an integer (0-4) or a character string:
#'   \itemize{
...
#'   }
#' @param max.iterations Maximum iterations for iterative methods. Default is 10.
#' @param convergence.threshold Convergence threshold. Default is 1e-6.
#' @param ikernel An integer (0-4) for kernel type. Default is 1.
#' @param dist.normalization.factor Distance normalization factor > 1. Default is 1.01.
#' @param n.CVs Number of cross-validation runs. Default is 0.
#' @param n.CV.folds Number of CV folds. Default is 10.
#' @param epsilon Numerical tolerance. Default is 1e-10.
#' @param seed Random seed for CV. Default is 0.
#'
...
#'
#' @export
graph.diffusion.matrix.smoother <- function(X,
                                           graph,
                                           edge.lengths,
                                           weights = NULL,
                                           n.time.steps = 100,
    ...
    if (!is.matrix(X)) {
        X <- as.matrix(X)
        if (!is.matrix(X)) {
            stop("'X' must be a matrix or coercible to matrix", call. = FALSE)
        }
    }
    ...
    if (n.vertices == 0 || n.features == 0) {
        stop("'X' must have at least one row and one column", call. = FALSE)
    }

    # Validate graph structure
    if (!is.list(graph) || length(graph) != n.vertices) {
        stop("'graph' must be a list with length equal to nrow(X)", call. = FALSE)
    }
    ...
    if (!is.list(edge.lengths) || length(edge.lengths) != n.vertices) {
        stop("'edge.lengths' must be a list with length equal to nrow(X)",
             call. = FALSE)
    }
    ...
    # Check graph and edge.lengths correspondence
    for (i in seq_len(n.vertices)) {
        if (length(graph[[i]]) != length(edge.lengths[[i]])) {
            stop(sprintf("graph[[%d]] and edge.lengths[[%d]] have different lengths",
                         i, i), call. = FALSE)
        }
    ...
    if (is.null(weights)) {
        ...
        if (!is.numeric(weights) || length(weights) != n.vertices) {
            stop("'weights' must be a numeric vector with length equal to nrow(X)",
                 call. = FALSE)
        }
    ...
    if (!is.numeric(n.time.steps) || length(n.time.steps) != 1 ||
        n.time.steps < 1 || round(n.time.steps) != n.time.steps) {
        stop("'n.time.steps' must be a positive integer", call. = FALSE)
    }

    if (!is.numeric(step.factor) || length(step.factor) != 1 ||
        step.factor <= 0 || step.factor > 1) {
        stop("'step.factor' must be a single value in (0, 1]", call. = FALSE)
    }
    ...
    # Handle imputation method
    if (is.character(imputation.method)) {
        valid_methods <- c("local_mean_threshold", "neighborhood_matching",
                           "iterative_neighborhood_matching", "supplied_threshold",
                           "global_mean_threshold")

        if (!(imputation.method %in% valid_methods)) {
            stop(sprintf("'imputation.method' must be one of: %s",
                         paste(valid_methods, collapse = ", ")), call. = FALSE)
        }

        imputation.method <- match(imputation.method, valid_methods) - 1L
    } else {
        if (!is.numeric(imputation.method) || length(imputation.method) != 1 ||
            !(imputation.method %in% 0:4)) {
    ...
    if (!is.numeric(max.iterations) || length(max.iterations) != 1 ||
        max.iterations < 1) {
        stop("'max.iterations' must be a positive number", call. = FALSE)
    }

    if (!is.numeric(convergence.threshold) || length(convergence.threshold) != 1 ||
        convergence.threshold <= 0) {
        stop("'convergence.threshold' must be a positive number", call. = FALSE)
    }

    if (!is.numeric(ikernel) || length(ikernel) != 1 || !(ikernel %in% 0:4)) {
    ...
    if (!is.numeric(n.CV.folds) || length(n.CV.folds) != 1 ||
        n.CV.folds <= 1 || round(n.CV.folds) != n.CV.folds) {
        stop("'n.CV.folds' must be an integer greater than 1", call. = FALSE)
    }
    ...
    result <- .Call("S_graph_diffusion_matrix_smoother",
                    X,
                    graph.0based,
                    edge.lengths,
                    weights,
                    as.integer(n.time.steps),
                    as.double(step.factor),
