...
#' Embed Graph in 2D Space
#'
#' @description
#' Computes a 2D layout for a graph using force-directed algorithms.
#'
#' @param adj_list Adjacency list representation of the graph
#' @param weights_list List of edge weights (optional)
#' @param invert_weights Whether to invert weights (useful when weights represent similarities)
#' @param dim Number of dimensions for the embedding (default: 2)
#' @param method Layout algorithm to use: "fr" for Fruchterman-Reingold or "kk" for Kamada-Kawai
#' @param verbose Whether to print progress information
#'
#' @return A matrix of coordinates with one row per vertex
#'
#' @details
#' This function implements force-directed graph layout algorithms to position
#' vertices in a 2D space. The goal is to position connected vertices close to
#' each other while keeping unconnected vertices far apart.
#'
#' @keywords internal
graph_embedding <- function(adj_list,
                            weights_list = NULL,
                            invert_weights = TRUE,
                            dim = 2,
                            method = c("fr", "kk"),
    ...
    for (j in seq_along(adj_list[[i]])) {
      ...
      if (i < neighbor) {
        ...
        if (!is.null(weights_list)) {
          w <- weights_list[[i]][j]
          if (invert_weights && w > 0) {
            w <- 1/w  ## Invert weight for layout (higher weight = closer vertices)
          }
          weights <- c(weights, w)
        }
      }
  ...
  ## If no edges, return random positions
  if (nrow(edges) == 0) {
    return(matrix(runif(n_vertices * dim, -1, 1), nrow = n_vertices, ncol = dim))
  }
  ...
  if (requireNamespace("igraph", quietly = TRUE)) {
    ...
    if (!is.null(weights_list) && length(weights) > 0) {
      g <- igraph::set_edge_attr(g, "weight", value = weights)
    }

    ## Compute layout
    if (method == "fr") {
      layout <- igraph::layout_with_fr(g, dim = dim)
    } else if (method == "kk") {
      layout <- igraph::layout_with_kk(g, dim = dim)
    }

    return(layout)
  } else {
    ## Fallback to a simple implementation if igraph is not available
    ## Initialize random positions
    positions <- matrix(runif(n_vertices * dim, -1, 1), nrow = n_vertices, ncol = dim)

    if (verbose) {
      warning("igraph package is not available. Using simple fallback layout method.")
    }
