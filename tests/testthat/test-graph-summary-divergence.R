test_that("compute.graph.summary.pmf returns expected degree PMF", {
  ...
  pmf <- .test.graph.summary("compute.graph.summary.pmf")(g, summary = "degree_distribution", return.details = FALSE)
...
test_that("component_size_distribution is vertex-weighted", {
  ...
  pmf <- .test.graph.summary("compute.graph.summary.pmf")(g, summary = "component_size_distribution", return.details = FALSE)
...
test_that("neighborhood_label_distribution summarizes undirected label pairs", {
  ...
  pmf <- .test.graph.summary("compute.graph.summary.pmf")(
    g,
...
test_that("edge_weight_distribution uses shared bins and returns zero divergence for identical graphs", {
  ...
  div <- .test.graph.summary("graph.summary.divergence")(
    g,
    g,
...
test_that("compute.graph.summary.stability preserves k values", {
  ...
  stab <- .test.graph.summary("compute.graph.summary.stability")(
...
test_that("compute.stability.metrics keeps legacy js.div field while using summary stability", {
  ...
  stab <- .test.graph.summary("compute.stability.metrics")(graphs, graph.type = "geom")
  deg.stab <- .test.graph.summary("compute.graph.summary.stability")(
    graphs = list(g1, g2, g3),
    summary = "degree_distribution",
    k.values = 3:5,
    return.details = TRUE
...
test_that("compute.stability.metrics supports alternate summary families through the higher-level API", {
  ...
  stab <- .test.graph.summary("compute.stability.metrics")(
    graphs,
    graph.type = "geom",
    summary = "neighborhood_label_distribution",
    labels = labels
  ...
  label.stab <- .test.graph.summary("compute.graph.summary.stability")(
    graphs = list(g1, g2, g3),
    summary = "neighborhood_label_distribution",
    labels = labels,
    k.values = 3:5,
