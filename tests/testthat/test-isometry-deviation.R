test_that("isometry helpers identify scaled exact distances", {
  ...
  expect_equal(dgraphs::isometry.scale(D.estimated, D.true), 0.5, tolerance = 1e-12)
  expect_equal(dgraphs::isometry.rel.rms.error(D.estimated, D.true), 0, tolerance = 1e-12)
  expect_equal(dgraphs::isometry.rel.abs.error(D.estimated, D.true), c(`50%` = 0, `95%` = 0),
               tolerance = 1e-12)
  expect_equal(
    dgraphs::isometry.distortion.quantiles(D.estimated, D.true),
    c(`5%` = 1, `50%` = 1, `95%` = 1),
    tolerance = 1e-12
  )
  expect_equal(dgraphs::isometry.distance.correlations(D.estimated, D.true),
               c(pearson_cor = 1, spearman_cor = 1),
               tolerance = 1e-12)
  expect_equal(
    dgraphs::isometry.geodesic.diagnostics(D.estimated, D.true),
    c(
...
test_that("summarize.isometry.deviation returns benchmark columns", {
  ...
  out <- dgraphs::summarize.isometry.deviation(D.estimated, D.true, scale = FALSE)
...
test_that("geodesic diagnostics report signed shortcut and detour behavior", {
  ...
  short <- dgraphs::isometry.geodesic.diagnostics(D.short, D.true, scale = FALSE)
  ...
  detour <- dgraphs::isometry.geodesic.diagnostics(D.long, D.true, scale = FALSE)
...
test_that("isometry helpers validate distance matrices", {
  ...
  expect_error(dgraphs::isometry.scale(D[1:2, ], D), "square")
  expect_error(dgraphs::isometry.scale(D, D[1:2, 1:2]), "same dimensions")
  D.bad <- D
  D.bad[1, 2] <- NA
  expect_error(dgraphs::isometry.scale(D.bad, D), "cannot contain")

  D.true <- matrix(1, 3, 3)
  diag(D.true) <- 0
  D.zero <- matrix(0, 3, 3)
  expect_error(dgraphs::isometry.scale(D.zero, D.true), "zero norm")
  expect_error(dgraphs::isometry.geodesic.diagnostics(D, D, band.probs = c(0.8, 0.2)),
               "band.probs")
})
